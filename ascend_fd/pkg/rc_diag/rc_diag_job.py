...
from ascend_fd.pkg.rc_diag.err_checker import (AllRankNoErrChecker, LostLogChecker,
                                               ErrorInfoChecker, NoErrInNFKChecker, Mode)
...
class RankTable:
    ...
    def add_relation(self, worker_id, rank_id, server_id, device_id):
        self.rank_map.update({rank_id: (worker_id, server_id, device_id)})
        self.server_device_map.update({f"{server_id}-{device_id}": (worker_id, rank_id)})

    def get_worker_server_device_from_rank(self, rank_id):
        return self.rank_map.get(rank_id, (-1, -1, -1))

    def get_worker_from_rank(self, rank_id):
        return self.get_worker_server_device_from_rank(rank_id)[0]

    def get_rank_from_server_device(self, server_id, device_id):
        return self.server_device_map.get(f"{server_id}-{device_id}", (-1, -1))[1]
...
class RCDiagWorker:
    ...
    def update_timeout(self, plog_file):
        """
    ...
    def add_plog_path(self, worker_id, plog_file):
        ...
        if not is_ok:
            ...
            return
        if self.rank_table.rank_num != -1 and rank_info_list[0] != self.rank_table.rank_num:
        ...
        self.rank_table.rank_num = rank_info_list[0]
        ...
        self.rank_table.add_relation(worker_id, *rank_info_list[1:])
        if is_error:
            self.rank_table.add_err_rank(rank_id)
        else:
            self.rank_table.add_no_err_rank(rank_id)
        self.plog_map.update({rank_id: plog_file})
